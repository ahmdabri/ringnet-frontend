...
function getLoginErrorMessage(err: unknown) {
  const error = err as { response?: { status?: number; data?: { message?: unknown } } };
  ...
  return String(error.response?.data?.message || "Login gagal. Periksa email dan password Anda.");
}
...
export default function LoginPage() {
  ...
  useEffect(() => {
    const token = window.localStorage.getItem("ringnet_token");
    if (token) {
