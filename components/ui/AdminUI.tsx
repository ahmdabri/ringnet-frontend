  ...
  return (
    <Card className="overflow-hidden">
      ...
      <div className="flex items-center justify-between border-t border-slate-200 px-4 py-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => setPage(Math.max(1, page - 1))} aria-label="Halaman sebelumnya" title="Halaman sebelumnya" className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-600 disabled:opacity-40" disabled={page === 1}><ChevronLeft size={16} /></button>
          <button onClick={() => setPage(Math.min(maxPage, page + 1))} aria-label="Halaman berikutnya" title="Halaman berikutnya" className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-600 disabled:opacity-40" disabled={page === maxPage}><ChevronRight size={16} /></button>
        </div>
      </div>
      {confirm ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => setConfirm(null)} aria-label="Tutup dialog hapus" title="Tutup dialog hapus" className="text-slate-400 hover:text-slate-700"><X size={18} /></button>
            </div>
