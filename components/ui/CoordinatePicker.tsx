...
export default function CoordinatePicker({ open, value, onClose, onSave }: CoordinatePickerProps) {
  ...
  useEffect(() => {
    geometry.tiles.forEach((tile) => {
      const tileElement = tileRefs.current.get(tile.key);
      if (!tileElement) return;
      tileElement.style.left = `${tile.left}px`;
      tileElement.style.top = `${tile.top}px`;
    });

    const markerElement = markerRef.current;
    if (markerElement && marker) {
    ...
    <div className="fixed inset-0 z-[100] bg-slate-950/55 p-4 backdrop-blur-sm">
      <div className="mx-auto flex h-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
          ...
          <button type="button" onClick={onClose} aria-label="Tutup pemilih koordinat" title="Tutup pemilih koordinat" className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-500 hover:text-slate-900">
            <X size={16} />
          </button>
        </div>
        ...
        >
          ...
          {geometry.tiles.map((tile) => (
            <div
              ...
              ref={(element) => {
                if (element) {
                  tileRefs.current.set(tile.key, element);
                } else {
            ...
            >
              <div className="relative h-64 w-64 select-none">
                <Image src={tile.src} alt="" fill unoptimized draggable={false} className="select-none object-cover" />
              </div>
            </div>
          ))}
          {marker ? (
            <div ref={markerRef} className="pointer-events-none absolute -translate-x-1/2 -translate-y-full text-rose-500 drop-shadow-lg">
          ...
          <div className="absolute left-4 top-24 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg">
            <button type="button" onClick={() => updateZoom(zoom + 1)} aria-label="Perbesar peta" title="Perbesar peta" className="grid h-10 w-10 place-items-center border-b border-slate-200 text-slate-700 hover:bg-slate-50">
              <Plus size={18} />
            </button>
            <button type="button" onClick={() => updateZoom(zoom - 1)} aria-label="Perkecil peta" title="Perkecil peta" className="grid h-10 w-10 place-items-center text-slate-700 hover:bg-slate-50">
              <Minus size={18} />
            </button>
          </div>
        </div>
