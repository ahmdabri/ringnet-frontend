...
export function InvoiceForm({ edit = false, id }: { edit?: boolean; id?: string }) {
  ...
  function invoiceCode(prefix: string) {
    ...
    const seed = `${prefix}-${year}-${month}-${form.customer_id || form.customer_name || form.service_type || form.invoice_name || "invoice"}`;
    const suffix = Array.from(seed).reduce((hash, char, index) => (hash + char.charCodeAt(0) * (index + 1)) % 1_000_000, 0);
    return `${prefix}/${year}/${month}/${String(suffix).padStart(6, "0")}`;
  }
